...
class CustomBrowser(Browser):
    ...
    async def _setup_builtin_browser(self, playwright: Playwright) -> PlaywrightBrowser:
        ...
        if (
            ...
            screen_size = (
                self.config.new_context_config.browser_window_size.model_dump()
            )
        ...
        logger.warning("type: %s", self.config.browser_class)
        logger.warning("chrome_args: %s", chrome_args)
        logger.warning("self.config.proxy: %s", self.config.proxy)
        ...
        #     handle_sigint=False,
        # )
        browser = await browser_class.connect(
            "wss://playwright.lab.athenachat.bot/playwright"
        )
        return browser
